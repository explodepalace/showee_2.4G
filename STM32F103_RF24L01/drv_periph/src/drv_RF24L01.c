...
uint8_t NRF24L01_Read_Reg( uint8_t RegAddr )
{
    ...
    drv_spi_read_write_byte( NRF_READ_REG | RegAddr );	//读命令地址
...
void NRF24L01_Write_Reg( uint8_t RegAddr, uint8_t Value )
{
    ...
    drv_spi_read_write_byte( NRF_WRITE_REG | RegAddr );	//写命令地址
...
{
    uint8_t btmp;
    uint8_t length = ( len > 32 ) ? 32 : len;		//数据长度大约32，则只发送32个字节
...
void NRF24L01_Gpio_Init( void )
{
	...
	RCC_APB2PeriphClockCmd( RF24L01_CE_GPIO_CLK | RF24L01_IRQ_GPIO_CLK, ENABLE );	//打开 CE IRQ 时钟，CS SCK MISO MOSI 由 SPI 初始化
	...
	RF24L01_SET_CE_LOW( );		//关闭24L01
	RF24L01_SET_CS_HIGH( );		//SPI片选拉高
}
...
void RF24L01_Init( void )
{
    ...
    NRF24L01_Write_Reg( SETUP_RETR, ARD_500US | 0x05 );         	//重复等待时间 500us, 重传5次
